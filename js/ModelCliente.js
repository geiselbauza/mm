(function($) {
    var self =  $.mobile.ModelCliente = {
        ...
        visiblecoord: ko.observable(false),
        visiblemapa:ko.observable(false),
	  ...
	  formatCurrency: function(value) {
		...
		return "BsS" + value.toFixed(2);
         ...
         AsignarCliente:  function()
          ...
          self.longitud(0); //colocar el valor de la tabla 
          self.latitud(0); //colocar el valor de la tabla   
        
          if (self.longitud()+self.latitud() == 0) self.visiblecoord(true);
          if (self.longitud()+self.latitud() != 0) self.visiblemapa(false);
        ...
        clickpuntear:  function() //aqui vamos a desarrollar el punteo de clientes   VERIFICAR SI GPS ON!
           {   
               if ("geolocation" in navigator){ //check geolocation available 
                ...
                alert("Se encontró su ubicación en latitud "+position.coords.latitude+" longitud :"+ position.coords.longitude);
             if (self.longitud()+self.latitud() != 0) self.visiblecoord(false);
             if (self.longitud()+self.latitud() != 0) self.visiblemapa(false);
             if (self.longitud()+self.latitud() != 0) self.visibleboton(true);

               
                });
                }else{
                alert("No se puede inicializar geolocalizacion");
                }
       ...
       //     if (self.longitud()+self.latitud() != 0) self.visiblecoord(true);
          ...
          var boton = document.getElementById('mapp');
            if (boton.textContent == 'Mostrar Ubicacion') {
                  boton.textContent = 'Ocultar ubicacion';
                  self.visiblemapa(true);
            }
            else{
                  boton.textContent = 'Mostrar Ubicacion'; 
                  self.visiblemapa(false); 
            } 
        ...
        if ($.mobile.ModelMenu.menuSelec() == '13')
                $.mobile.changePage('#vercliente');
      ...
      $.mobile.SqLite.transaction
        (
          function(tx)
            ...
            if (($.mobile.ModelMenu.menuSelec()=="1") || ($.mobile.ModelMenu.menuSelec()=="10") || ($.mobile.ModelMenu.menuSelec()=="12"))// Lista de Cliente si es para pedidos...
            {
